...
exports.DEFAULT_LAY_FAVORITE_CRITERIA = exports.DEFAULT_LAY_00_CRITERIA = void 0;
exports.checkLay00 = checkLay00;
exports.checkLayFavorite = checkLayFavorite;
exports.calculateConfidence = calculateConfidence;
exports.analyzeMatch = analyzeMatch;
exports.sortMatches = sortMatches;
exports.filterAndSortMatches = filterAndSortMatches;
/**
 * Critérios padrão documentados
 */
exports.DEFAULT_LAY_00_CRITERIA = {
    goalsAvg65Threshold: 0.85,
    htGoalRateThreshold: 80,
    over15RateThreshold: 85,
};
exports.DEFAULT_LAY_FAVORITE_CRITERIA = {
 ...
 */
function checkLay00(homeStats, awayStats, criteria = exports.DEFAULT_LAY_00_CRITERIA) {
    // Uma das equipes com gols até 65 min >= 0.85
    const goalsAvg65Met = homeStats.goalsAvg65 >= criteria.goalsAvg65Threshold ||
        awayStats.goalsAvg65 >= criteria.goalsAvg65Threshold;
    if (!goalsAvg65Met)
        return false;
    // Ambas com HT Goals >= 80%
    const htGoalRateMet = homeStats.htGoalRate >= criteria.htGoalRateThreshold &&
        awayStats.htGoalRate >= criteria.htGoalRateThreshold;
    if (!htGoalRateMet)
        return false;
    // Ambas com Over 1.5 >= 85%
    const over15RateMet = homeStats.over15Rate >= criteria.over15RateThreshold &&
 ...
 */
function determineFavorite(homeStats, awayStats) {
 ...
 */
function checkLayFavorite(homeStats, awayStats, criteria = exports.DEFAULT_LAY_FAVORITE_CRITERIA) {
    const isFavoriteHome = homeStats.goalsAvg70 >= awayStats.goalsAvg70;
    ...
    const favoriteGoalsAvg70Met = favoriteStats.goalsAvg70 >=
        criteria.favoriteGoalsAvg70Threshold;
    if (!favoriteGoalsAvg70Met)
        return false;
    // Adversário: concededAvg65 >= 0.85
    const opponentConcededAvg65Met = opponentStats.concededAvg65 >=
        criteria.opponentConcededAvg65Threshold;
    if (!opponentConcededAvg65Met)
        return false;
    // Critérios opcionais (reforçam a recomendação)
    let bonusScore = 0;
    if (criteria.scoredFrequencyThreshold &&
        favoriteStats.scoredFrequency >=
            criteria.scoredFrequencyThreshold) {
        bonusScore += 10;
    }
    if (criteria.over15Threshold &&
 ...
 */
function calculateConfidence(homeStats, awayStats, method) {
    let score = 50; // Base
    if (method === "LAY_00") {
        // Quanto maior o goalsAvg65, melhor
        const avgGoalsScore = Math.min((Math.max(homeStats.goalsAvg65, awayStats.goalsAvg65) /
            0.85) *
            15, 15);
        score += avgGoalsScore;
        // HT Goals
        const htScore = Math.min((Math.max(homeStats.htGoalRate, awayStats.htGoalRate) /
            80) *
            15, 15);
        score += htScore;
        // Over 1.5
        const overScore = Math.min((Math.max(homeStats.over15Rate, awayStats.over15Rate) /
            85) *
            20, 20);
        score += overScore;
    }
        ...
        const opponentStats = isFavoriteHome ? awayStats : homeStats;
        // Favorito goals
        const favGoalsScore = Math.min((favoriteStats.goalsAvg70 / 0.85) * 20, 20);
        score += favGoalsScore;
        // Opponent conceded
        const oppScore = Math.min((opponentStats.concededAvg65 / 0.85) * 20, 20);
        score += oppScore;
        // Bonus
...
function sortMatches(matches) {
    return [...matches].sort((a, b) => {
        ...
        const maxGoalsA = Math.max(a.homeStats.goalsAvg70, a.awayStats.goalsAvg70);
        const maxGoalsB = Math.max(b.homeStats.goalsAvg70, b.awayStats.goalsAvg70);
